...
class SupabaseTournamentRepository(
    ...
    override suspend fun scheduleMatch(matchId: String, scheduledAt: Long): Result<Unit> = try {
        val response = api.updateTournamentSwissMatch(
            id = matchId,
            body = mapOf("scheduled_at" to java.time.Instant.ofEpochMilli(scheduledAt).toString())
        )
        if (response.isSuccessful) Result.success(Unit)
        else Result.failure(Exception("Failed to schedule match"))
    } catch (e: Exception) {
        Result.failure(e)
    }
